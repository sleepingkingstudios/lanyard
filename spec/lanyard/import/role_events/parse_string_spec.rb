...
require 'lanyard/import/role_events/parse_string'
...
RSpec.describe Lanyard::Import::RoleEvents::ParseString do
  ...
  describe '#call' do
    ...
    describe 'with an invalid event date' do
      ...
      let(:error_message) { 'invalid date 2024, 2, 30' }
    ...
    %w[Jan Feb Mar Apr May Jun].each.with_index do |month, month_index|
      describe "with a valid event in #{month}" do
        let(:raw_value)  { "Applied #{month} 1" }
        let(:event_date) { Date.new(2024, 1 + month_index, 1) }
        let(:expected_value) do
          {
            'event_date' => event_date,
            'type'       => RoleEvents::AppliedEvent.name
    ...
    %w[Jul Aug Sep Oct Nov Dec].each.with_index do |month, month_index|
      describe "with a valid event in #{month}" do
        let(:raw_value)  { "Applied #{month} 1" }
        let(:event_date) { Date.new(2023, 7 + month_index, 1) }
        let(:expected_value) do
          {
            'event_date' => event_date,
            'type'       => RoleEvents::AppliedEvent.name
