...
RSpec.describe Lanyard::Actions::Applications::Update, type: :action do
  ...
  let(:valid_attributes) do
    {
      'source'    => Application::Sources::LINKEDIN,
      'job_title' => 'Senior Executive Vice President of Title Generation'
    }
  end
  let(:application) { FactoryBot.create(:application, :with_job_search) }

  include_contract 'update action contract',
    existing_entity:    -> { application },
  ...
  do
    describe 'with id: a slug' do
      let(:params) do
        { 'id' => application.slug, 'application' => valid_attributes }
      end
