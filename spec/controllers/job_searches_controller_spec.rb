...
RSpec.describe JobSearchesController, type: :controller do
  ...
  describe '.resource' do
    ...
    it { expect(resource.base_path).to be == '/job-searches' }

    it { expect(resource.default_order).to be :start_date }

    it { expect(resource.permitted_attributes).to be == permitted_attributes }

    it { expect(resource.resource_class).to be == JobSearch }

    it { expect(resource.resource_name).to be == 'job_searches' }

    it 'should define the block component' do
      expect(resource.block_component)
        .to be Lanyard::View::Components::JobSearches::Block
    end

    it 'should define the form component' do
      expect(resource.form_component)
        .to be Lanyard::View::Components::JobSearches::Form
    end

    it 'should define the table component' do
      expect(resource.table_component)
        .to be Lanyard::View::Components::JobSearches::Table
    end
  end
  ...
  include_contract 'should define action',
    :create,
    Lanyard::Actions::JobSearches::Create,
    member: false
  ...
  include_contract 'should define action',
    :update,
    Lanyard::Actions::JobSearches::Update,
    member: true
end
